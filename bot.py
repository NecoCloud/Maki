...
#######################################################################################################################
    ...
    async def cmd_help(self, command=None):
        ...
        if command:
            ...
            helpmsg += "```\n"
            helpmsg += ";help [command]를 하시면 개별 커맨드를 설명 해드려요.".format(self.config.command_prefix)
    ...
    async def cmd_id(self, author, user_mentions):
        ...
        """
        if not user_mentions:
            return Response('당신의 아이디는 `%s`' % author.id, reply=True, delete_after=35)
        else:
            usr = user_mentions[0]
    ...
    async def cmd_joinserver(self, message, server_link=None):
        ...
        if self.user.bot:
            ...
            return Response(
                "봇 계정은 초대 링크를 사용할 수 없어요! 저를 서버에 추가하려면 링크를 클릭해주세요.: \n{}".format(url),
    ...
    async def cmd_summon(self, channel, server, author, voice_channel):
        ...
        if not chperms.connect:
            ...
            return Response(
                "```채널에 참여할수 없어요. \"{}\", 저에게 권한이 없어요.```".format(author.voice_channel.name),
                delete_after=25
            )
            ...
            return Response(
                "```채널에 가입되지 않았어요. \"{}\", 저에게 말할 권한이 없어요.```".format(author.voice_channel.name),
                delete_after=25
            )
